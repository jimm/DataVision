 ...
 * @param user the user name to use when logging in to the database
 */
...
// ----- BEGIN: Change as per patch #1463294 -----
...
// ----- END: Change as per patch #1463294 -----
}
...
protected Table findTable(String tableName) {
    ...
    String origTableName = tableName; // Preserve passed-in value for map key
	...
	if ((t = findTableWithId(schemaName + '.' + tableName)) != null) {
	    tableCacheMap.put(origTableName, t);
	    return t;
	}
    }
    ...
    // table's schema name.
    if (name != null) {
	String dbSchemaName = name;
	if (!getReport().caseSensitiveDatabaseNames())
	    dbSchemaName = dbSchemaName.toLowerCase();

	if (!dbSchemaName.equals(schemaName)) {
	    if ((t = findTableWithId(dbSchemaName + '.' + tableName)) != null)
	    {
		tableCacheMap.put(origTableName, t);
		return t;
	    }
    ...
    // Finally, try with just the table name.
...
/**
 * Finds table with given <var>id</var> string. Returns <code>null</code> if
 * it is not found. If the report says that database names are not
 * case-sensitive, then we do a case-insensitive comparison.
 *
 * @param id a table id; if names are not case-sensitive then <var>id</var>
 * will be lower-case when it is passed in
 */
	   ...
	   IllegalAccessException, UserCancellationException
{
	...
	try {
	    ...
	    if (username == null) // Allow empty username
...
protected void loadAllTables() throws SQLException {
    ...
    quoteString = dbmd.getIdentifierQuoteString();
